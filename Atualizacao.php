...
<?php
  ...
  /* Coletando Informações e Jogando Dentro de Variáveis */
	...
	/* Apresentando Informações Antigas

	$query = mysqli_query($conexao, "SELECT * FROM usuarios WHERE nome='$antigonome'");

	while($result = mysqli_fetch_array($query)){
  	echo "Informações Alteradas com Sucesso! <br>
  		<br>

  		Suas Antigas Informações: <br>
  		Id: $result[id] <br>
   		Nome: $result[nome] <br>
      	Senha: $result[senha] <br>
      	Email: $result[email] <br>
      	-------------------------- <br>";
  } */
 
  /* Fazendo a Atualização das Informações */

   	$query = mysqli_query($conexao, "UPDATE usuarios SET nome = '$novonome' senha = '$novasenha' email = '$novoemail' WHERE nome = '$antigonome'");

  /* Apresentando as Novas Informações */

   	$query = mysqli_query($conexao, "SELECT * FROM usuarios WHERE nome='$novonome'");

	  while($result = mysqli_fetch_array($query)){
  
  echo "Suas Novas Informações:

    <table border='1' cellpading='10px' bgcolor='#C0C0C0' width='60%'>
    <tr>
        <th> ID  </th> 
        <th> Nome  </th>
        <th> Senha  </th>

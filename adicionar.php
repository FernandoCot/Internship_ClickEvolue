...
<?php
  ...
  /* Coletando as Informações e Salvando em Variáveis */

  $nome = $_POST['nomeins'];
  $senha = $_POST['senha'];
  $email = $_POST['email'];

  $query = mysqli_query($conexao, "INSERT INTO usuarios (nome, senha, email) values ('$nome', '$senha', '$email')");

  $query = mysqli_query($conexao, "SELECT * FROM usuarios WHERE nome = '$nome'");

  while($result = mysqli_fetch_array($query)){
  
  echo "<br> <br> <h2>Os Seguintes Dados Foram Inseridos no Banco: </h2><br> <br>

    <table border='1' cellpading='15px' width='60%' bgcolor='#C0C0C0'>
    <tr>
        <th> ID  </th> 
        <th> Nome  </th>
        <th> Senha  </th>
